    ...
    def run(self):
        ...
        for activity in activities:
            if activityaction.has_key(activity) and activityaction[activity] == 'android.intent.action.MAIN':
                ...
                call(['monkeyrunner', 'scripts/monkeyrunner.py', apkPath+apkName, runPackage, runActivity], stderr=PIPE)
...
Path = sys.argv[1]
PathList = Path.split('/')
apkName = PathList[-1:][0]
apkPath = '/'.join(PathList[0:-1]) + '/'
raw = fd.read()
fd.close()
...
f = open('../AnalysisInformation/'+apkName+'_Information.txt','w+')
print ''
f.write('\n')
space = ' ' * 5
space2 = ' ' * 8
...
savefig('../AnalysisImages/'+apkName+"_behaviorgraph.png")
...
Size = F.get_size_inches()
savefig('../AnalysisImages/'+apkName+'_tree.png', bbox_inches = 'tight', pad_inches = 2.0)
